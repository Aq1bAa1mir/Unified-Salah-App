...
// Add collapsible functionality for ayah section
function initAyahCollapse() {
    const ayahHeader = document.getElementById('ayahHeader');
    const ayahSection = document.querySelector('.ayah-section');
    const ayahContent = document.getElementById('ayahContent');
    
    if (ayahHeader && ayahSection && ayahContent) {
        // Set initial height
        setAyahContentHeight();
        
        // Handle both click and touch events for mobile compatibility
        const toggleAyahSection = () => {
            if (ayahSection.classList.contains('collapsed')) {
                // Expanding
                expandAyahSection();
            } else {
                // Collapsing
        ...
        ayahHeader.addEventListener('click', toggleAyahSection);
        ayahHeader.addEventListener('touchend', (e) => {
...
// Expand ayah section
function expandAyahSection() {
    const ayahSection = document.querySelector('.ayah-section');
    const ayahContent = document.getElementById('ayahContent');
    
    if (ayahSection && ayahContent) {
        ayahSection.classList.remove('collapsed');
        
        // Set the height to the actual content height
        setAyahContentHeight();
    }
}

// Collapse ayah section
function collapseAyahSection() {
    const ayahSection = document.querySelector('.ayah-section');
    const ayahContent = document.getElementById('ayahContent');
    
    if (ayahSection && ayahContent) {
        // First set a specific height so the transition works
        const contentHeight = ayahContent.scrollHeight;
        ayahContent.style.maxHeight = contentHeight + 'px';
        
        // Force a reflow
        ayahContent.offsetHeight;
        
        // Then add collapsed class which sets max-height to 0
        ayahSection.classList.add('collapsed');
    }
}

// Dynamically set ayah content height based on actual content
function setAyahContentHeight() {
    const ayahContent = document.getElementById('ayahContent');
    if (ayahContent) {
        // Temporarily remove height restrictions to measure content
        const currentMaxHeight = ayahContent.style.maxHeight;
        ayahContent.style.maxHeight = 'none';
        
        // Measure the actual content height
        const contentHeight = ayahContent.scrollHeight;
        
        // Set the max-height to the measured height for smooth animation
        ayahContent.style.maxHeight = contentHeight + 'px';
    }
}
...
// Register our Service Worker and clean up any legacy ones
if ('serviceWorker' in navigator) {
  window.addEventListener('load', async () => {
    try {
      // Unregister legacy PWABuilder SW if present
      const regs = await navigator.serviceWorker.getRegistrations();
      for (const r of regs) {
        if (r.active?.scriptURL?.includes('pwabuilder-sw')) {
          console.log('[SW] Unregistering legacy PWABuilder SW');
          await r.unregister();
        }
      }
...
// Notification scheduling: ask permission and send prayer reminders (20 min before and at time)
...
async function schedulePrayerNotificationsForToday(times){
    try{
        const granted = await ensureNotificationPermission();
        if(!granted) return;
        if(!navigator.serviceWorker?.controller){
            // ping once SW is ready
            navigator.serviceWorker.ready.then(() => schedulePrayerNotificationsForToday(times));
            return;
        }

        const toMinutes = t=>{
            // supports "HH:MM" and "HH:MM AM/PM"
            if(!t) return null;
            if(t.includes('AM')||t.includes('PM')){ return convertToMinutes(t); }
            const [hh,mm]=t.split(':').map(Number); return hh*60+mm;
        };

        const scheduleFor = (name, tStr)=>{
            const mins = toMinutes(tStr);
            if(mins==null) return;
            navigator.serviceWorker.controller?.postMessage({
                type:'SCHEDULE_PRAYER',
                payload:{ name, minutes: mins }
            });
        };

        scheduleFor('Fajr', times.fajr);
        scheduleFor('Dhuhr', times.dhuhr);
        scheduleFor('Asr', times.asr);
        scheduleFor('Maghrib', times.maghrib);
        scheduleFor('Isha', times.isha);
        console.log('[Notify] Schedules sent to SW');
    }catch(e){ console.error('[Notify] schedule error', e); }
}

// Hook into updates to send schedules after fetching today's times
const _origUpdateNextPrayer = updateNextPrayer;
updateNextPrayer = function(data){
    _origUpdateNextPrayer(data);
    // Only schedule when viewing today
    const today = new Date();
    if(isSameDay(selectedDate, today)){
        schedulePrayerNotificationsForToday(data);
    }
};

// Keep schedules fresh even if SW sleeps
setInterval(() => {
    const today = new Date();
    if (isSameDay(selectedDate, today) && window.latestPrayerTimesForToday) {
        schedulePrayerNotificationsForToday(window.latestPrayerTimesForToday);
    }
}, 60 * 1000);
